...
5. Added a "Cost Per Day" calculated column
   ...
   - I will use this to look at the variation between billing periods using a metric that factors in the number of days in each period.
6. Removed the following columns from the table: statement date, bill period end
   - I want to work with the “Bill Period Start” column rather than “Statement Date.” The former represents the actual period while the latter is unrelated to bill cost.
...
- Many year to year changes were extreme. October - January all saw increases greater than 45%.
...
- There appear to be large increases a the start of each season, followed by decreases or little change.
- No season has consistently higher or lower usage, but there was a steady period of increases from August - December 2023. The highest bills all occured from November 2023 - March 2024.
- Used line charts, trendlines, conditional formatting (color scales), and percent change calculations (pivot tables)
...
- Overall increased total bill costs despite frequent ups and down.
...
1. Though utility bill totals fluctuate on a monthly basis, the overall trend is increasing.
2. In the coming year, total bill amounts will likely exceed current average of $70. Expect bills ranging from $80-$100.
...
- Develop a contingency budget
   - Save to accomodate unexpected spikes in costs.
- Conduct monthly data review and update
   - Adding each new month's data to the data set will provide more information for analysis and prediciton. This can allow for more accurate forecasting in the future.
- Implement community conservation practices to reduce water usage during Fall and Winter months including:
   ...
   - Reminders prior to holidays about utility usage mindfulness
   - Urge residents to inspect appliances and plumbing, informing maintance of leaks or inefficient operation.
